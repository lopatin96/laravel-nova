...
class PaidOrders extends Partition
{
    ...
    public function calculate(NovaRequest $request): PartitionResult
    {
        ...
        return $this->count(
            $request,
            $this->query
                ? $this->query->where('user_id', '!=', 1)->whereIn('status', [OrderStatus::Completed, OrderStatus::Processed])
                : Order::where('user_id', '!=', 1)->whereIn('status', [OrderStatus::Completed, OrderStatus::Processed]), 'product_id'
        )
